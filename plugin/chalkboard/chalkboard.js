...
******************************************************************/
	...
	function configure( config ) {
	    ...
	    if ( config.slideWidth ) slideWidth = config.slideWidth;
	    if ( config.slideHeight ) slideHeight = config.slideHeight;
		...
		switch ( theme ) {
			case "whiteboard":
				...
				pens = [ boardmarkers, boardmarkers ];
		                grid = { color: 'rgb(127,127,255,0.1)', distance: 40, width: 1};
				break;
			case "chalkboard":
			default:
...
//console.log("showChalkboard");
	...
	function drawGrid() {
		...
		drawingCanvas[1].yOffset = (drawingCanvas[1].height - storage[1].height * drawingCanvas[1].scale)/2;
                ...
                var slideLeft = (drawingCanvas[1].width - slideWidth)/2; 
                var slideTop = (drawingCanvas[1].height - slideHeight)/2;           
		...
		for( var y=fromY; y < drawingCanvas[1].height; y+=distance ) {
  		...
  		context.stroke();
            ...
            context.beginPath();            
            context.lineWidth = 1;
            context.strokeStyle = 'rgba(127,127,127,.9)';
            context.fillStyle = 'rgba(127,127,127,.1)';
            context.rect(slideLeft, slideTop, slideWidth, slideHeight);
            //context.fillRect(slideLeft, slideTop, slideWidth, slideHeight);            
            context.stroke();
	}
